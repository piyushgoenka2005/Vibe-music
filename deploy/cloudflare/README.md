...
After DNS is proxied and `cf-ray` appears on responses, run on the VPS:

```bash
sudo CLOUDFLARE_ONLY=1 bash deploy/complete-audit-go-live.sh
```

This deploys latest code, syncs nginx real-IP config, and locks the origin firewall (L-22/L-23).
