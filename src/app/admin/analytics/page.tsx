    ...
    <>
      <div className="admin-toolbar">
        ...
        <button
          ...
          onClick={() => {
            downloadFromApi(`/api/admin/analytics/export?period=${period}`);
          }}
        >
          Export CSV
        </button>
      ...
      <div className="admin-stat-grid">
        ...
        <StatCard
          ...
          value={Math.round(report.averageOrderValue)}
          format="currency"
        />
      </div>
      ...
      <div className="admin-grid-2" style={{ marginTop: "1.5rem" }}>
        <div className="admin-panel">
          <div className="admin-panel__header">
            <h2 className="admin-panel__title">Revenue Trend</h2>
          </div>
          <div className="admin-panel__body">
      ...
      <div className="admin-panel" style={{ marginTop: "1.5rem" }}>
        ...
        >
          {Object.entries(report.ordersByStatus as Record<string, number>).map(
            ([status, count]) => (
              <div
                key={status}
                style={{
              ...
              {webhookData.metrics.recentFailures?.length > 0 ? (
                <div className="admin-table-wrap">
                  <table className="admin-table">
                    ...
                    <tbody>
                      {webhookData.metrics.recentFailures.map(
                        (row: {
                          id: string;
                          eventType: string;
                          orderId?: string | null;
