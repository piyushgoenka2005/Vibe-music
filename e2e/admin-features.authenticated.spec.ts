...
test.describe("Admin password reset full flow", () => {
  ...
  test("request reset, set new password, login, restore password", async ({
    page,
    request,
  }) => {
...
test.describe("Product image fit (PDP)", () => {
  ...
  test("gallery image uses object-fit contain on mobile viewport", async ({
    page,
    request,
  }) => {
    ...
    const photo = page.locator(".pdp-gallery__photo").first();
